...
import com.bsuir.lab2.filter.CustomFilter;
...
public class Runner {
	...
	public static void main(String args[]) {
		...
		if (Arrays.asList(args).contains(SourceType.XML.toString())) {
			storage.setAllTransport(FactoryBuilder.getInstance().build(new XMLFactory()));
		} else if (Arrays.asList(args).contains(SourceType.JSON.toString())) {
			storage.setAllTransport(FactoryBuilder.getInstance().build(new JSONFactory()));
		} else {
		...
		customFilter.execute(storage);
		if (storage.getAllTransport() != null && !storage.getAllTransport().isEmpty())
			toReport(storage);

	}

	private static void toFilter(TransportStorage storage) {
		CustomFilter filer = new CustomFilter();
		filer.execute(storage);
	}
